package org.smm.archetype.infrastructure.common.search;
...
public class SearchServiceImpl implements SearchService {
    ...
    @Override
    public <T> org.smm.archetype.domain.common.search.result.AiSearchResult<T> aiSearch(
            String index,
            org.smm.archetype.domain.common.search.query.AiSearchQuery query,
            Class<T> documentClass) {
        try {
            // 构建AI搜索查询DSL（包含重排序策略）
    ...
    public <T> SearchResult<T> hybridSearch(
            String index,
            org.smm.archetype.domain.common.search.query.HybridSearchQuery query,
            Class<T> documentClass) {
        try {
            // 构建混合搜索查询DSL（BM25 + kNN）
