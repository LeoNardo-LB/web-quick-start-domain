...
public class LogAspect {
    ...
    public Object doAround(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        // 获取日志上下文（统一处理类型和业务描述）
        LogContext logContext = resolveLogContext(myLog, declaringType);
     ...
     * 使用策略模式思想，提高可维护性和可读性
     * @param myLog         方法上的@MyLog注解
     * @param declaringType 方法声明类型
     * @return 日志上下文
     */
    private LogContext resolveLogContext(MyLog myLog, Class<?> declaringType) {
        // 优先级1: 有@MyLog注解 -> 方法执行日志
        if (myLog != null) {
            return new LogContext("方法执行", myLog.value());
        }
