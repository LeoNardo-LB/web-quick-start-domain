 ...
 * <pre>{@code
 * // 字符串转枚举（带默认值）
 * EventStatus status = EnumUtils.fromString("CREATED", EventStatus.class, EventStatus.CREATED);
 *
 * // 字符串转枚举（异常模式）
 * EventPriority priority = EnumUtils.fromStringStrict("HIGH", EventPriority.class);
 *
 * // 数值转枚举
 * ConsumeStatus consumeStatus = EnumUtils.fromValue(1, ConsumeStatus.class);
 * }</pre>
 * @author Leonardo
 * @since 2026/01/09
 */
