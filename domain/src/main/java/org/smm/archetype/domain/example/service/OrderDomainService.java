...
public class OrderDomainService {
     ...
     * 验证订单项
     * @param items 订单项列表
     * @throws IllegalArgumentException 订单项无效
     */
    ...
    private void validateOrderItem(OrderItem item) {
        ...
        if (item.getUnitPrice() == null || item.getUnitPrice().isNegative()) {
            throw new IllegalArgumentException("单价必须大于等于0");
        }
    ...
    public void validateInventory(List<OrderItemInfo> orderItemInfos)
        ...
        System.out.println("============ OrderDomainService.validateInventory called ============");
        System.out.println("Inventory service: " + inventoryService.getClass().getName());
        System.out.println("OrderItemInfos: " + orderItemInfos);
        ...
        System.out.println("Converted inventory items: " + inventoryItems);

        if (!inventoryService.validateInventory(inventoryItems)) {
            System.out.println("Inventory validation returned FALSE");
            throw new InventoryService.InsufficientInventoryException("库存不足，无法创建订单");
        }

        System.out.println("Inventory validation returned TRUE");
     ...
     * @throws IllegalArgumentException 金额不匹配
     */
