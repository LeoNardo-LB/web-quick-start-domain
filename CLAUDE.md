...
**命名规则**：按DDD分包划分，格式为 `{模块名}_dev_log.md`

### 🔄 记录流程

#### 触发时机
AI完成用户的一个需求后，会询问：
> 📋 是否需要将本次开发记录到 `_dev_log/[module]_dev_log.md`？
...
```bash
# 查找所有新功能记录
grep "#新功能" _dev_log/*.md

# 查找订单模块的优化记录
grep "#优化" _dev_log/order_dev_log.md
```

**按日期查找**：
```bash
# 查找特定日期的记录
grep "## 2026-01-15" _dev_log/*.md
```

**按文件查找**：
```bash
# 查找涉及某文件的所有记录
grep "OrderSnapshot.java" _dev_log/*.md
```
